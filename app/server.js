...
class io_listenToPlayer extends IO {
  ...
  think() {
    ...
    if (this.player.command.reversedtank) {   
      targ = {
        x: -this.player.target.x,
        y: -this.player.target.y,
      };
    ...
    return {
      ...
      main:
        ...
        this.player.command.reversedtank || 
...
/*class io_twistRepel extends IO {
  constructor(b) {
    super(b);
  }
  ...
  think(input) {
    if (input.alt) {
      return{
        facing: this.facing -= -0.1 / roomSpeed,
        main: true,
      } 
    } else if (input.main){
      return{
...
class io_nearestDomiMaster extends IO {
  constructor(body) {
    super(body);
    this.targetLock = undefined;
    this.tick = ran.irandom(30);
  ...
  think(input) {
    ...
    if (this.targetLock != null && this.targetLock.valid()) {
      ...
      return {
        target: {
          x: diff.x + this.lead * radial.x * 100, //degree of predictive shooting
          y: diff.y + this.lead * radial.y * 100,
        },
        fire: true,
        main: true,
      };
...
class io_sniperminion extends IO {
  constructor(body) {
    super(body);
    this.turnwise = 1;
  }
...
class io_rammerMinion extends IO {
  constructor(body) {
    super(body);
    this.turnwise = 1;
  }
  ...
  think(input) {
    ...
    if (input.target != null && (input.alt || input.main)) {
      let sizeFactor = Math.sqrt(this.body.master.size / this.body.master.SIZE);
      let leash = 0 * sizeFactor;
      let orbit = 0 * sizeFactor;
      let repel = 0 * sizeFactor;
      let goal;
      let power = 1;
      let target = new Vector(input.target.x, input.target.y);
      if (input.alt) {
...
class io_wanderLust extends IO {
  ...
  CheckArea(range) {
        ...
        let out = entities.map(e => {
            ...
            if (e.master.master.team !== -101) {
            if (e.master.master.team !== this.body.master.master.team) {
            if (e.type === 'tank' || e.type === 'spike' || e.type === 'crasher' || e.type === 'food' || e.type === 'gemcrasher') {
...
class io_superslowspin extends IO {
  constructor(b) {
    super(b);
    this.a = 0;
  }
...
class io_dominatorTurn extends IO {
  constructor(b) {
    super(b);
  }
...
class io_fleeAtLowHealthBack extends IO {
  constructor(b) {
    super(b);
    this.fear = util.clamp(ran.gauss(0.7, 0.15), 0.1, 0.9);
  }
...
class io_wingL extends IO {
  ...
  think(input) {
    ...
    if(input.main){
      return {                
        target: {
          x: Math.cos(Math.PI/8+Math.PI*Math.sin(this.a)/4+this.body.source.facing),
          y: Math.sin(Math.PI/8+Math.PI*Math.sin(this.a)/4+this.body.source.facing),
        },
        main: true
      };
    }
      ...
      return {
        target: {
          x: Math.cos(Math.PI*19/36+this.body.source.facing),
          y: Math.sin(Math.PI*19/36+this.body.source.facing)
        },
        main: true
      }
    }
...
class io_wingR extends IO {
  ...
  think(input) {
    ...
    if(input.main){
      return {                
        target: {
          x: Math.cos(Math.PI*15/8+Math.PI*Math.sin(this.a)/4+this.body.source.facing),
          y: Math.sin(Math.PI*15/8+Math.PI*Math.sin(this.a)/4+this.body.source.facing),
        },
        main: true
      };
    }
      ...
      return {
        target: {
          x: Math.cos(Math.PI*53/36+this.body.source.facing),
          y: Math.sin(Math.PI*53/36+this.body.source.facing)
        },
        main: true
      }
    }
        ...
        if (this.cycle < 1) {
          this.cycle +=
            ...
            if (shootPermission && (this.autofire || ((this.altFire) ? this.body.control.alt : this.body.control.fire))) {
                  if (this.cycle >= 1) {
                    if ((this.countsOwnKids) ? (this.countsOwnKids-1) < this.children.length : (this.body.maxChildren) ? (this.body.maxChildren-1) < this.body.children.length : false) {
                      if (this.destroyOldestChild) {
                          this.destroyOldest();
                      }
                    }
  ...
  bulletInit(o) {
    ...
    o.necro = (host) => {
      ...
      if (shootPermission) {
        ...
        host.SIZE = save.size;
        host.health.max = 0.15*host.health.amount;
        return true;
      }
      return false;
    };
    ...
    o.gondo = (host) => {
      ...
      if (shootPermission) {
        ...
        host.color = o.color;
        host.facing = save.facing;
        host.SIZE = save.size+(0.50*save.size);
        host.health.max = 0.15*host.health.amount;
        return true;
      }
      return false;
    ...
    o.mininecro = (host) => {
      ...
      if (shootPermission) {
        ...
        host.facing = save.facing;
        host.SIZE = save.size-(0.50*save.size);
        host.health.max = 0.08*host.health.amount;
        return true;
      }
      return false;
    };
    ...
    o.penta = (host) => {
      ...
      if (shootPermission) {
        ...
        host.facing = save.facing;
        host.SIZE = save.size;
        host.health.max = 0.06*host.health.amount;
        return true;
      }
      return false;
    };
...
class Entity {
  constructor(position, master = this) {
    ...
    this.cameraOverrideFoV = null;
    this.solitaireScale = 1;
  ...
  define(set) {
    ...
    if (set.COLOR != null) {
      this.color = set.COLOR;
        ...
        if (this.type === 'squareWall'){
       switch (c.WALL_SETUP) {
            case "normal":
            {this.color = 16
              break;}
            case "apartment":    
              switch (ran.chooseChance(2, 1, 0.01)) {
                case 0: 
                  this.color = 16
                  break;
                case 1: 
                  this.color = ran.choose([6,7,8,9,17,18,19,25])
                  break;
                case 2: 
                  this.color = 36
                  break;
           ...
           break;
            case "pumpkin":
            {this.color = ran.choose([39,40,41])
              break;
             }
            case "halloween":
            {this.color = ran.choose([19,30,40])
              break;
             }
            case "cave":    
            {this.color = ran.choose([9,14,17,19,30,31])
                break;
             }
            case "valentine":    
            {this.color = ran.choose([5,12,18,39,39,39])
                break;
             }
            case "danger":    
            {this.color = ran.choose([3,9,12,13,16,17,19,32,35])
                break;
             }
          　case "easter": 
             switch (ran.chooseChance(3.5, 1)) {
                case 0: 
                  this.color = ran.choose([1,3,6,33])
                  break;
                case 1: 
                  this.color = ran.choose([37,0])
                  break;
             }
            break;
           
            case "christmas":
              switch (ran.chooseChance(2, 1, 0.5)) {
                case 0: 
                  this.color = ran.choose([39,41])
                  break;
                case 1: 
                  this.color = 12
                  break;
                case 2: 
                  this.color = ran.choose([8,35])
                  break;
             }
            break;
            ...
            else if(wallcontrol == 2){
              this.color = ran.choose([3,9,12,13,16,17,19,32,35])
                break;}//danger
            else if(wallcontrol == 3){
              this.color = ran.choose([5,12,18,39,39,39])
                break;}//valentine
            else if(wallcontrol == 4){
              this.color = ran.choose([9,14,17,19,30,31])
                break;}//cave
            else if(wallcontrol == 5){
              this.color = ran.choose([19,30,40])
                break;}//halloween
            else if(wallcontrol == 6){
              this.color = ran.choose([39,40,41])
                break;}//pumpkin   
            else if(wallcontrol == 7)
            {switch (ran.chooseChance(2, 1, 0.01)) {
                case 0: 
                  this.color = 16
                  break;
        ...
        //normal,apartment,pumpkin,halloween,cave,valentine,danger,easter,christmas
    };
    if (this.type === 'wall'){
     switch (c.ROID_SETUP) {
       case "normal":
        ...
        switch (ran.chooseChance(2, 1)) {
          case 0: 
            this.color = 16
              break;
          case 1: 
            this.color = ran.choose([7,17,18,25])
              break;
        }
          break;
            case "pumpkin":
         ...
         break;
     } //normal,obstaclecolor,pumpkin,halloween,cave,valentine,danger,easter,christmas
    };
    ...
    if (set.PENTA != null) {
      this.settings.isPentamancer = set.PENTA;
    }
    if (set.GONDO != null) {
      this.settings.isGondola = set.GONDO;
    }
    if (set.MININECRO != null) {
      this.settings.isMiniNecro = set.MININECRO;
    }
    ...
    if (set.ZOOM_SCALING != null) {
        this.solitaireScale = set.ZOOM_SCALING
    }
  ...
  camera(tur = false) {
    return {
      type:
        ...
        (this.type === "tank" || this.type === "spike" || this.type === "IndicateInMap" || this.type === "reflectsize") * 0x04 +
        this.invuln * 0x08,
      id: this.id,
      index: this.index,
      x: this.x,
      ...
      layer:
        this.bond != null
          ...
          : this.type === "food"
          ? 10
          : this.type === "tank" || this.type === "spike" || this.type === "IndicateInMap" || this.type === "reflectsize"
          ? 5
          : this.type === "crasher"
          ? 1
          : 0,
  ...
  takePhoto() {
    return {
      ...
      name: this.type === "tank" 
           || this.type === "spike"
           || this.type === "IndicateInMap"
           || this.type === "reflectsize"
             ? this.name : "",
      index: this.index,
      color: this.color,
      layer:
  ...
  upgrade(number) {
    if (
      ...
      if (typeof saveMe.TOOLTIP === "string")
  ...
  move() {
    ...
    switch (this.motionType) {
      ...
      case "bigbiggrow":
        ...
        break;
      case "circle":
        this.x = 120 * Math.sin(this.facing) + 3120;
        this.y = 120 * Math.cos(this.facing) + 240;
        this.facing += 10*Math.PI/180
        break;
      case "orbit":
        // Define the maximum radius and the time it takes to reach that radius
        const maxNormalRadius = 180; // You can adjust this value to change the maximum normal radius
        const maxIncreasedRadius = 450; // Maximum radius when holding this.control.main
        const timeToMaxRadius = 60; // Adjust this value to control the time it takes to reach maxRadius
        const angularSpeed = 5 * (Math.PI / 180); // Angular speed in radians per frame

        // Calculate the rate of increase for the radius
        const radiusIncrement = maxNormalRadius / timeToMaxRadius;

        // Initialize maxRadius to the normal maximum radius
        let maxRadius = maxNormalRadius;

        // Increment the time
        this.time = this.time || 0;
        this.time++;

        // Flag to track if maxRadius has been increased
        let radiusIncreased = false;

        // Check if this.control.main is being held
        if (this.control.main) {
          if (!radiusIncreased) {
            // Only set maxRadius to maxIncreasedRadius if it's not already set
            maxRadius = maxIncreasedRadius;
            radiusIncreased = true; // Set the flag to true
          }
        } else if (maxRadius > maxNormalRadius) {
          // Gradually reduce maxRadius to maxNormalRadius
          maxRadius = Math.max(maxRadius-1, maxNormalRadius);
          // Reset the flag when decreasing
          radiusIncreased = false;
        }

        // Calculate the current radius
        const currentRadius = Math.min(this.time * radiusIncrement, maxRadius);

        // Update the position of the object
        this.x = this.master.x + currentRadius * Math.cos(this.facing);
        this.y = this.master.y + currentRadius * Math.sin(this.facing);

        // Decrement the facing angle to create reverse rotation
        this.facing += angularSpeed;

        // Stop increasing the radius once it reaches the defined maximum
        if (currentRadius >= maxRadius) {
          this.time = timeToMaxRadius; // Optional: Stop time from increasing further
        }
        break;
      case "reverseOrbit":
        // Define the maximum radius and the time it takes to reach that radius
        ...
        if (this.control.main) {
          if (!radiusIncreased2) {
            // Only set maxRadius to maxIncreasedRadius if it's not already set
            maxRadius = maxIncreasedRadius;
            radiusIncreased2 = true; // Set the flag to true
          }
        } else if (maxRadius > maxNormalRadius) {
          // Gradually reduce maxRadius to maxNormalRadius
          maxRadius = Math.max(maxRadius-1, maxNormalRadius);
          // Reset the flag when decreasing
          radiusIncreased2 = false;
        }
      ...
      case "motor":
        ...
        break;
      case "oscillate": //thanks to DeepRecovery for these oscillate codes
        this.maxSpeed = this.topSpeed;
        this.damp = 0;
        if (Math.abs(this.velocity.x) > Math.abs(this.velocity.y))
            this.y += 10 * Math.cos(this.range/1.3);
        else
            this.x += 10 * Math.cos(this.range/1.3);
      break;
      case "oscillate2":
        this.maxSpeed = this.topSpeed;
        this.damp = 0;
        if (Math.abs(this.velocity.x) > Math.abs(this.velocity.y))
            this.y -= 10 * Math.sin(this.range/1.3);
        else
            this.x -= 10 * Math.sin(this.range/1.3);
      break;
      case "swarm":
        this.maxSpeed = this.topSpeed;
  ...
  face() {
    ...
    switch (this.facingType) {
      ...
      case "smoothToTarget":
        ...
        break;
      case 'twisterSpin':
        if (this.control.alt) this.facing += -0.1 / roomSpeed
        else this.facing += 0.1 / roomSpeed;
      break;
      case "bound":
        let givenangle;
        if (this.control.main) {
  ...
  confinementToTheseEarthlyShackles() {
    ...
    if (room.gameMode === "tdm" && this.type !== "food" && this.bond == null) {
      let loc = { x: this.x, y: this.y };
        if (this.label !== "back" && room.isIn("lacl", loc)) {this.accel.x-=7}
        if (this.label !== "back" && room.isIn("racl", loc)) {this.accel.x+=7}
        if (this.label !== "back" && room.isIn("uacl", loc)) {this.accel.y-=7}
        if (this.label !== "back" && room.isIn("dacl", loc)) {this.accel.y+=7}
        if (this.type === "reflectsize" && room.isIn("bigy", loc) ||
            this.type === "miniboss" && room.isIn("bigy", loc) ||
            this.type === "crasher" && room.isIn("bigy", loc) ||
            this.type === "minion" && room.isIn("bigy", loc) ||
            this.type === "spike" && room.isIn("bigy", loc) ||
            this.type === "tank" && room.isIn("bigy", loc)
        ) {this.coreSize=50} 
      let teleport = (into,out)	=> {
        if (!room[out] || !room[out].length) return undefined;
        if(this.teleinterval<=0){
          if (
            this.type === "tank" && room.isIn(into, loc) ||
            this.type === "spike" && room.isIn(into, loc) ||
            this.type === "reflectsize" && room.isIn(into, loc) ||
            this.type === "crasher" && room.isIn(into, loc) ||
            this.type === "miniboss" && room.isIn(into, loc) ||
            this.type === "minion" && room.isIn(into, loc) ||  
            this.type === "drone" && room.isIn(into, loc) ||
            this.type === "healdrone" && room.isIn(into, loc) ||
            this.type === "healbullet" && room.isIn(into, loc)
          )
       {
        let selection = room[out][ran.irandom(room[out].length - 1)]; 
      this.x =
        //ran.irandom((0.5 * room.width) / room.xgrid) * ran.choose([-1, 1]) +
        selection.x,
      this.y =
        //ran.irandom((0.5 * room.height) / room.ygrid) * ran.choose([-1, 1]) +
        selection.y
        this.teleaccelx = ran.irandom((0.02 * room.width) / room.xgrid) * ran.choose([-1, 1])
        this.teleaccely = ran.irandom((0.02 * room.height) / room.ygrid) * ran.choose([-1, 1])
        this.teleinterval = 500;
         }
        }
        if(this.teleinterval>=0)
     if ((room.isIn(into, loc))) {
          this.accel.x += this.teleaccelx
          this.accel.y += this.teleaccely
           }
          this.teleinterval--;
};
      if (
      ...
      if(
        this.type === "food" && room.isIn("port", loc) ||
        this.type === "food" && room.isIn("por1", loc) ||
        this.type === "food" && room.isIn("por2", loc) ||
        this.type === "food" && room.isIn("por3", loc) ||
        this.type === "food" && room.isIn("por4", loc) ||
        this.type === "food" && room.isIn("1por", loc) ||
        this.type === "food" && room.isIn("2por", loc) ||
        this.type === "food" && room.isIn("3por", loc) ||
        this.type === "food" && room.isIn("4por", loc) ||
        this.type === "food" && room.isIn("bpo1", loc) ||
        this.type === "food" && room.isIn("bpo2", loc) ||        
        this.type === "food" && room.isIn("bpo3", loc) ||
        this.type === "food" && room.isIn("bpo4", loc) ||
        this.type === "bullet" && room.isIn("port", loc) ||
        this.type === "bullet" && room.isIn("por1", loc) ||
        this.type === "bullet" && room.isIn("por2", loc) ||
        this.type === "bullet" && room.isIn("por3", loc) ||
        this.type === "bullet" && room.isIn("por4", loc) ||
        this.type === "bullet" && room.isIn("1por", loc) ||
        this.type === "bullet" && room.isIn("2por", loc) ||
        this.type === "bullet" && room.isIn("3por", loc) ||
        this.type === "bullet" && room.isIn("4por", loc) ||
        this.type === "bullet" && room.isIn("bpo1", loc) ||
        this.type === "bullet" && room.isIn("bpo2", loc) ||        
        this.type === "bullet" && room.isIn("bpo3", loc) ||
        this.type === "bullet" && room.isIn("bpo4", loc) ||
        this.type === "swarm" && room.isIn("port", loc) ||
        this.type === "swarm" && room.isIn("por1", loc) ||
        this.type === "swarm" && room.isIn("por2", loc) ||
        this.type === "swarm" && room.isIn("por3", loc) ||
        this.type === "swarm" && room.isIn("por4", loc) ||
        this.type === "swarm" && room.isIn("1por", loc) ||
        this.type === "swarm" && room.isIn("2por", loc) ||
        this.type === "swarm" && room.isIn("3por", loc) ||
        this.type === "swarm" && room.isIn("4por", loc) ||
        this.type === "swarm" && room.isIn("bpo1", loc) ||
        this.type === "swarm" && room.isIn("bpo2", loc) ||        
        this.type === "swarm" && room.isIn("bpo3", loc) ||
        this.type === "swarm" && room.isIn("bpo4", loc) ||
        this.type === "trap" && room.isIn("port", loc) ||
        this.type === "trap" && room.isIn("por1", loc) ||
        this.type === "trap" && room.isIn("por2", loc) ||
        this.type === "trap" && room.isIn("por3", loc) ||
        this.type === "trap" && room.isIn("por4", loc) ||
        this.type === "trap" && room.isIn("1por", loc) ||
        this.type === "trap" && room.isIn("2por", loc) ||
        this.type === "trap" && room.isIn("3por", loc) ||
        this.type === "trap" && room.isIn("4por", loc) ||
        this.type === "trap" && room.isIn("bpo1", loc) ||
        this.type === "trap" && room.isIn("bpo2", loc) ||        
        this.type === "trap" && room.isIn("bpo3", loc) ||
        this.type === "trap" && room.isIn("bpo4", loc) ||
        this.type === "block" && room.isIn("port", loc) ||
        this.type === "block" && room.isIn("por1", loc) ||
        this.type === "block" && room.isIn("por2", loc) ||
        this.type === "block" && room.isIn("por3", loc) ||
        this.type === "block" && room.isIn("por4", loc) ||
        this.type === "block" && room.isIn("1por", loc) ||
        this.type === "block" && room.isIn("2por", loc) ||
        this.type === "block" && room.isIn("3por", loc) ||
        this.type === "block" && room.isIn("4por", loc) ||
        this.type === "block" && room.isIn("bpo1", loc) ||
        this.type === "block" && room.isIn("bpo2", loc) ||        
        this.type === "block" && room.isIn("bpo3", loc) ||
        this.type === "block" && room.isIn("bpo4", loc)
      ) { this.kill(); }
      teleport("por1","1por")
      teleport("1por",ran.choose(["port","port","por1"]))
      teleport("por2","2por")
      teleport("2por",ran.choose(["port","port","por2"]))
      teleport("por3","3por")
      teleport("3por",ran.choose(["port","port","por3"]))
      teleport("por4","4por")
      teleport("4por",ran.choose(["port","port","por4"]))
      teleport("port",ran.choose(["bpo1","bpo2","bpo3","bpo4"]))
      teleport("bpo1",ran.choose(["port","bpo2","bpo3","bpo4"]))
      teleport("bpo2",ran.choose(["port","bpo1","bpo3","bpo4"]))
      teleport("bpo3",ran.choose(["port","bpo1","bpo2","bpo4"]))
      teleport("bpo4",ran.choose(["port","bpo1","bpo2","bpo3"]))
    }
  }
    /*if (room.gameMode === "tdm" && this.type !== "food" && this.bond == null) {
      let loc = { x: this.x, y: this.y };
      if (
        ...
        (this.team !== -2 && room.isIn("bas2", loc)) ||
        (this.team !== -3 && room.isIn("bas3", loc)) ||
        (this.team !== -1 && room.isIn("bas4", loc)) ||
        (this.team !== -2 && room.isIn("bap1", loc)) ||
        (this.team !== -3 && room.isIn("bap2", loc)) ||
        (this.team !== -3 && room.isIn("bap3", loc)) ||
        (this.team !== -4 && room.isIn("bap4", loc))
      ) { this.damageRecieved = 0; }
    }
  ...
  contemplationOfMortality() {
   ...
   this.collisionArray.forEach((instance) => {
    if (instance.master.type === "spike" && this.type === "healbullet" || instance.master.type === "tank" && this.type === "healbullet" || instance.master.type === "reflectsize"  && this.type === "healbullet" ||
        instance.master.type === "spike" && this.type === "healdrone" || instance.master.type === "tank" && this.type === "healdrone" || instance.master.type === "reflectsize"  && this.type === "healdrone"){
       this.master.skill.score += 100
    }
      })
    }
    this.damageRecieved = 0;
    ...
    if (this.isDead()) {
      ...
      let name =
        this.master.name == ""
          ? this.master.type === "tank" || this.master.type === "spike"
      ...
      this.collisionArray.forEach((instance) => {
        ...
        if (instance.master.settings.acceptsScore) {
          ...
          if (
            ...
            instance.master.type === "spike" ||
            instance.master.type === "miniboss" ||
            instance.master.type === "IndicateInMap"||
            instance.master.type === "reflectsize"
          )
            notJustFood = true;
      ...
      killers.forEach((instance) => {
        ...
        if (this.type === "tank" || this.type === "spike") {
          ...
          else if (this.type === "IndicateInMap") {instance.killCount.bosses++;}
          else if (this.type === "reflectsize") {
            if (this.label === "Mothership"){instance.killCount.bosses++;}
            else{if (killers.length > 1) instance.killCount.assists++;
                 else instance.killCount.solo++;}
          }
      });
  ...
  gazeUpon() {
    ...
    if (player.body != null) {
      ...
      if (player.body.isDead()) {
        ...
        if (
          process.env.SECRET !== "" &&
          this.socket.key === process.env.SECRET
        ) {
          this.socket.talk("F", ...player.records(), 0);
        } else {
          this.socket.talk(
            "F",
            ...player.records(),
            c.RESPAWN_TIMER * 1000 - 5000.5
          );
        }
        // Remove the body
        player.body = null;
  ...
  sync(player) {
    let syncPhoto = (p1, p2, tur) => {
      ...
      if (player.body && p2.masterId === player.body.id) {
        ...
        if (player.command.reversedtank) {
          p2.type |= 0x02;
        }
      }
    ...
    if (this.lastUpdate - this.lastVisibleUpdate > c.visibleListInterval) {
      ...
      this.visibleEntity.forEach((e) => {
        if (!e.valid() || !this.isInView(e) || e.isDead()) {
          this.remove(e);
        }
      });
        ...
        let minimapAll = new Delta(5, () => {
          ...
          for (let my of entities) {
            ...
            if (
              ...
              my.type === "IndicateInMap" ||
              (my.type === "tank" && my.lifetime || my.type === "spike" && my.lifetime)
            ) {
              all.push({
                id: my.id,
                data: [
                  my.type === "wall" || my.type === "squareWall"
                    ? my.shape === 4
                      ? 2
                      : 1
                    : 0,
            ...
            if (
              ...
              all.push({
                id: my.id,
                data: [
                  my.type === "reflectsize" || my.type === "miniboss"
                    ? my.shape === 4
                      ? 2
                      : 1
                    : 0,
        ...
        let leaderboard = new Delta(5, () => {
          ...
          for (let instance of entities)
            if (
              ...
              (instance.type === "tank" || 
               instance.type === "spike" ||
               instance.type === "IndicateInMap" ||
               instance.type === "reflectsize" ||
               instance.killCount.solo ||
               instance.killCount.assists)
            ) {
              list.push(instance);
  ...
  let collide = (() => {
      ...
      if (component >= diff.length - combinedRadius) {
        ...
        if (goahead) {
          ...
          if (doDamage) {
            ...
            if (
              my.shape === n.shape &&
              my.settings.isGondola &&
              n.type === "food"
            ) {
              bail = my.gondo(n);
            } else if (
              my.shape === n.shape &&
              n.settings.isGondola &&
              my.type === "food"
            ) {
              bail = n.gondo(my);
            }
            if (
              my.shape === n.shape &&
              my.settings.isMiniNecro &&
              n.type === "food"
            ) {
              bail = my.mininecro(n);
            } else if (
              my.shape === n.shape &&
              n.settings.isMiniNecro &&
              my.type === "food"
            ) {
              bail = n.mininecro(my);
            }
            if (
              my.shape === n.shape &&
              my.settings.isPentamancer &&
              n.type === "food"
            ) {
              bail = my.penta(n);
            } else if (
              my.shape === n.shape &&
              n.settings.isPentamancer &&
              my.type === "food"
            ) {
              bail = n.penta(my);
            }
    ...
    function squarecollide(my, n) {
      ...
      if (
        ...
        let dist_x = Math.abs(dest.x - (my.x + my.size * grid_x));
        if (dist_x < n.size) {
          n.accel.x += (n.size - dist_x) * grid_x;
          kill = true;
        }
      } else if (
        (grid_x === 0 && grid_y === -1) ||
        ...
        let dist_y = Math.abs(dest.y - (my.y + my.size * grid_y));
        if (dist_y < n.size) {
          n.accel.y += (n.size - dist_y) * grid_y;
          kill = true;
        }
      } else {
        let rad = Math.atan2(dest.x - my.x, dest.y - my.y);
        n.accel.x += Math.sin(rad);
        n.accel.y += Math.cos(rad);
        kill = true;
      }
    ...
    return (collision) => {
      ...
      if (instance.type === "squareWall" || other.type === "squareWall") {
        if (!instance.intangibility && !other.intangibility){
          if (instance.type === "squareWall") squarecollide(instance, other);
          else squarecollide(other, instance);
        }
      }
      // Handle walls
      else if (instance.type === 'wall' || other.type === 'wall' || instance.type === "squareWall" || other.type === "squareWall") {
          if(instance.GoesThroughWalls === true || other.GoesThroughWalls === true || instance.GoesThroughAlls === true || other.GoesThroughAlls ===true) return;
          let a = instance.type === "bullet" || other.type === "bullet" ||
              instance.type === "healbullet" ||  other.type === "healbullet" 
              ? 1 +
                10 /
      ...
      // Otherwise, collide normally if they're from different teams
        ...
        if (instance.type === "healbullet" ||　other.type === "healbullet" || instance.type === "healdrone" ||　other.type === "healdrone"){
         advancedcollide(other, instance, false, false);
        }
        else if (instance.type !== "base" && other.type !== "base")
        {advancedcollide(instance, other, true, true);}
        else if (instance.type === "base" || other.type === "base") {
        if (instance.type !== "bullet" && other.type !== "bullet" && 
            instance.type !== "healbullet" && other.type !== "healbullet") {
      ...
      else if (instance.type === "healbullet" ||　other.type === "healbullet" || instance.type === "healdrone" ||　other.type === "healdrone"){
         if (instance.team === other.team){
           if (instance.type === "tank" || other.type === "tank" ||
               instance.type === "spike" || other.type === "spike" ||
               instance.type === "reflectsize" || other.type === "reflectsize" ||
               instance.type === "IndicateInMap" || other.type === "IndicateInMap"){
             if(instance.health.amount < instance.health.max){
           advancedcollide(instance, other, true, true);
             }
           }
         }
      ...
      else if (instance.type === "tank" || other.type === "tank" ||
               instance.type === "miniboss" || other.type === "miniboss" ||
               instance.type === "reflectsize" || other.type === "reflectsize") {
        if (instance.type !== "spike" && other.type !== "spike" &&
            instance.type !== "bullet" && other.type !== "bullet" && 
            instance.type !== "healbullet" && other.type !== "healbullet" &&
            instance.type !== "healdrone" && other.type !== "healdrone" &&
            instance.type !== "drone" && other.type !== "drone" &&
            instance.type !== "swarm" && other.type !== "swarm" &&
            instance.type !== "trap" && other.type !== "trap" &&
            instance.type !== "minion" && other.type !== "minion") { 
            advancedcollide(other, instance, false, false, 1);
        }
      }
...
var maintainloop = (() => {
  ...
  function placeRoids() {
    ...
    for (let i = Math.ceil(roidcount); i; i--) {
      count++;
      placeRoid("roid", /*Class.obstacle*/ Class.XmasTree1);
    }
    for (let i = Math.ceil(roidcount * 0.3); i; i--) {
      count++;
      placeRoid("roid", /*Class.babyObstacle*/ Class.XmasTree2);
    }
  ...
  function placeWalles(col=[16],loca="wall",def="wall") {
    let f = (loc) => {
      let gridWidth = room.width / room.xgrid;      
      let o = new Entity(loc);
      o.define(Class[def]);
      o.color= ran.choose(col)
      o.SIZE = gridWidth / 2;
      o.coreSize = o.SIZE;
      o.protect();
      o.team = -101;
    ...
    if (room[loca]) {
      room[loca].forEach((loc) => {
        f(loc);
      });
    }
  }
  placeWalles();
  function placeEntity(info) {
    let grid = room.width/room.xgrid;
    let o = new Entity({x:info.x*grid-grid/2,y:info.y*grid-grid/2});
    o.define(Class[info.define])
    o.team = info.team == null ? -100 : info.team;
    if (info.color!=null) {o.color = info.color}
    if(info.size != null){o.SIZE *= info.size};
    if(info.facing != null) {o.facing = info.facing*Math.PI / 180};
    if (info.other!=null) {info.other(o)}
    o.refreshBodyAttributes();
  }
  room.refresh = () => {
    let j = 0;
  ...
  let spawnBosses = (() => {
    ...
    let boss = (() => {
      let i = 0,
        names = [],
        bois = [Class.eggsanctuary],//, Class.squaresanctuary, Class.pentagonsanctuary],
        n = 0,
    ...
    return (census) => {
      if (timer > 6000 && ran.dice(16000 - timer)) {
        ...
        switch (ran.chooseChance(40, 1)) {
          case 0:
            choice = [[Class.elite_destroyer], 0, "a", "nest"];
            break;
          case 1:
            choice = [[Class.palisade], 0, "castle", "norm"];
  ...
  let spawnCrasher = (census) => {
    if (
      ran.chance(
        1 - (0.08 * census.crasher) / room.maxFood / room.nestFoodAmount
      )
    ) {
      let spot,
        i = 30;
      ...
      let type = ran.dice(60)
        ? ran.choose([Class.megaSentry,  Class.megaCrusher, Class.sentrySwarm, Class.sentryGun, Class.sentryTrap, Class.flashSentry, Class.miniPulsar, Class.miniCollider, Class.miniBlade, Class.leaper])
        : ran.choose([Class.crasher, Class.longboi, Class.kamikaze, Class.blade, Class.crasher2, Class.crusher, Class.diamondCrasher, Class.wasp]);
      let o = new Entity(spot);
      o.define(type);
      o.team = -100;
    }
  ...
  let makenpcs = (() => {
    ...
    let f = (loc, team) => { 
        let o = new Entity(loc);
            ...
            o.define((ran.dice(50)) ? ran.choose([
              Class.destroyerDominator, 
              Class.gunnerDominator, 
              Class.factoryDominator, 
              Class.trapperDominator]) : ran.choose([Class.destroyerDominator, Class.gunnerDominator, Class.factoryDominator, Class.trapperDominator]));
            o.team = -team;
    ...
    placeWalles([36],"racl","racl")
    placeWalles([36],"lacl","lacl")
    placeWalles([36],"uacl","uacl")
    placeWalles([36],"dacl","dacl")
    placeWalles([5],"bigy","bigy")
        ...
        return () => {
            ...
            // Bots
                if (bots.length < c.BOTS) {
                    ...
                    o.define((ran.dice(80)) ? ran.choose([
                      Class.giantbasic]) : ran.choose([
                        Class.twinbro, Class.sniperbro, Class.machinebro, Class.flankbro, Class.pelleterbro, Class.shieldprotect,
                        Class.admin, Class.starship, Class.poundbro, Class.trapperbro, Class.sibling, Class.kage, Class.bigbrother,
                        Class.dualshielder, Class.trishielder, Class.autoshielder, Class.hybridshielder, 
                        Class.shielderswarm, Class.scutum, Class.duo, Class.sharpshooter, Class.gadgetgun, 
                        Class.termion, Class.coordinator, Class.bruiser, Class.tricker, Class.mono, 
                        Class.autosingle, Class.bulwark, Class.musket, Class.tripletwin, Class.quadtwin, Class.split, 
                        Class.doubleguard, Class.doubletwinary, Class.autodouble, Class.bentdouble, 
                        Class.triplettwin, Class.flankgunner, Class.penta, Class.hepta, Class.spread, 
                        Class.bentgunner, Class.splitbent, Class.bentmini, Class.revenger, Class.benthybrid, 
                        Class.trinary, Class.triplemachine, Class.triple, Class.quint, Class.nailgun, Class.rimfire, 
                        Class.autogunner, Class.volley, Class.ferreter, Class.pursuer, Class.battery, 
                        Class.machinegunner, Class.sunburst, Class.guntrap, Class.overgunner, Class.starmachine, 
                        Class.starsniper, Class.terminator, Class.stari, Class.octo, Class.whirlwind, Class.hurricane, 
                        Class.tornado, Class.spreader , Class.scattergun, Class.trap4, Class.overpenta, 
                        Class.hexatrap, Class.dual, Class.autotwinary, Class.hybridtwinary, Class.bushwhack, 
                        Class.clinker, Class.basher, Class.puncher, Class.clipper, Class.ranger, Class.stalker, 
                        Class.carnivore, Class.enforce, Class.harvester, Class.falcon, Class.autoass, 
                        Class.buttbuttin, Class.cateran, Class.sniper3, Class.hhunter, Class.preda, 
                        Class.infantryman, Class.poach, Class.ordnance, Class.faucile, Class.crossbow, 
                        Class.silencer, Class.tririfle, Class.autorifle, Class.armsman, Class.armswarm, 
                        Class.arrower, Class.orion, Class.twinlaser, Class.nucleotide, Class.cultivator, 
                        Class.beamer, Class.laserswarm, Class.bentsniper, Class.sounder, Class.jalopy, 
                        Class.foamer, Class.silo, Class.plower, Class.dieseltrapper, Class.polluter, 
                        Class.autodiesel, Class.furnace, Class.splasher, Class.triblaster, Class.autoblaster, 
                        Class.swarmblaster, Class.hybridblaster, Class.chain, Class.gatlingspray, Class.accurator, 
                        Class.twingatling, Class.trigatling, Class.mitrailleuse, Class.halfnhalf, Class.swarmgatling, 
                        Class.hybridgatling, Class.mortar, Class.beekeeper, Class.sheller, Class.foctillery, 
                        Class.forger, Class.fieldgun, Class.blare, Class.discharger, Class.biggun, Class.stream, 
                        Class.cascader, Class.streamsnipe, Class.subverter, Class.streammachine, Class.punter, 
                        Class.hybridmini, Class.minitrap, Class.contender, Class.taser, Class.magnum, 
                        Class.zipper, Class.trimachinertrap, Class.triploid, Class.cicada, Class.machine3, 
                        Class.naturalist, Class.lonploid, Class.diploid, Class.beegunner, Class.flamethrower, 
                        Class.redistributor, Class.atomizer, Class.flamer2, Class.quarry, Class.shotgun2, 
                        Class.birdshot, Class.trapshot, Class.swarmshot, Class.automultishot, Class.multigun, 
                        Class.insect, Class.archaea, Class.pellelada, Class.crab, Class.competitor, 
                        Class.autoarachnid, Class.fighter, Class.booster, Class.brutalizer, Class.shocker, 
                        Class.bomber, Class.eagle2, Class.autotri, Class.heavy3, Class.auto4, Class.auto5, 
                        Class.cauto4, Class.combo, Class.banshee, Class.pirate, Class.shuriken, Class.twif, 
                        Class.snipef, Class.machinef, Class.peltef, Class.poundof, Class.trappef, Class.bushwhack, 
                        Class.holster, Class.swardian, Class.conq, Class.piercer, Class.belt, Class.triborer, 
                        Class.railgun, Class.sailor, Class.groundwire, Class.vulcan, Class.gust, Class.sheath, 
                        Class.annexer, Class.subhybridmachine, Class.submachineswarmer, Class.driller, 
                        Class.screwswarmer, Class.triscrewdriver, Class.autoscrewdriver, Class.hybridscrewdriver, 
                        Class.pentapelleter, Class.spacecraft, Class.manager, Class.grimreaper, Class.honker, 
                        Class.viper, Class.rocketstar, Class.torpedo, Class.senter, Class.overclock, Class.overkill, 
                        Class.homing, Class.baltimore, Class.gondola, Class.honchodrive, Class.hongen, 
                        Class.minister, Class.protectorate, Class.stovepipe, Class.box, Class.tributary, 
                        Class.autocolony, Class.codrive, Class.drive, Class.swarmdrive, Class.underdrive, 
                        Class.directorstorm, Class.autodirectordrive, Class.opiate, Class.spawnive, Class.megacraft, 
                        Class.twincraft, Class.triplecraft, Class.battlecraft, Class.autocraft, Class.hybridcraft, 
                        Class.swarmcraft, Class.sfig, Class.dualgen, Class.machogen, Class.noviciate, 
                        Class.necrogen, Class.spitfire, Class.microbe, Class.factory, Class.spawnicitor, 
                        Class.laborer, Class.producer, Class.animator, Class.octospawn, Class.lilhybrid, 
                        Class.hangar, Class.airship, Class.manufacturer, Class.overlord, Class.overtrap, 
                        Class.commander , Class.necromancer, Class.maleficitor, Class.infector, Class.trimancer, 
                        Class.pentamancer, Class.gemancer, Class.enchantress, Class.prodigy, Class.elamficitor, 
                        Class.autocruiser, Class.carrier, Class.warship, Class.battleship, Class.submarine, 
                        Class.fortress, Class.loopshipper, Class.dropshipper, Class.bentshipper, Class.autodropship, 
                        Class.swarmdropship, Class.hybriddropship, Class.defeater, Class.trilauncher, 
                        Class.flankgrower, Class.anni, Class.xdestroy, Class.destrodrive, Class.slinker, 
                        Class.hybrid, Class.construct, Class.bumper, Class.blower, Class.launchermini, 
                        Class.promenader, Class.skimmer, Class.twister, Class.rocketeer, Class.hiveshooter, 
                        Class.sidewind, Class.spark, Class.nova, Class.bazooka, Class.corps, Class.hanabi, 
                        Class.cracker, Class.boombadier, Class.megagrower, Class.twingrower, 
                        Class.machinegrower, Class.snarer, Class.autobuilder, Class.buildtrap, Class.stall, 
                        Class.swarmbuilder, Class.architect, Class.quadtrapper, Class.engineer, Class.charger, 
                        Class.boomer, Class.bentboomer, Class.blocker, Class.swarmboom, 
                        Class.megamachinetrapper, Class.mechatrapper, Class.encircle, Class.automatrap, 
                        Class.swarmmatrap, Class.thermostat, Class.twinarsenal, Class.triarsenal, Class.operator, 
                        Class.autoarsenal, Class.arsenicitor, Class.traplet, Class.hewntrap, Class.waller, 
                        Class.mswarmtrap, Class.autoswarmtrap, Class.hstrap, Class.septatrap, Class.mtrap3, 
                        Class.tripen, Class.gigatrapper, Class.automegatrap, Class.autopen, Class.interner,
                        //Class.mhealer, Class.trihealer, Class.medic, Class.ambulance, Class.ivs, Class.cure, Class.surgeon, Class.overhaul, Class.sister, 
                    ]));
                ...
                // Remove dead ones
                bots2 = bots2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bots2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                bots3 = bots3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bots3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                // Remove dead ones
                bots4 = bots4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bots4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                if (sbots1.length < c.SBOTS1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.tbot2);
                    o.team = -1;
                    //o.define(Class.basic);
                    o.define((ran.dice(80)) ? ran.choose([
                      Class.landmine, Class.megasmash, Class.spike, Class.weirdspike, Class.autosmash, Class.smashbro, Class.jumpsmash, Class.auger, Class.bonker, Class.flail,]) 
                             : ran.choose([Class.landmine, Class.megasmash, Class.spike, Class.weirdspike, Class.autosmash, Class.smashbro, Class.jumpsmash, Class.auger, Class.bonker, Class.flail,
                          ]));
                    o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    sbots1.push(o);
                ...
                // Remove dead ones
                sbots1 = sbots1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sbots1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                // Remove dead ones
                sbots2 = sbots2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sbots2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                // Remove dead ones
                sbots3 = sbots3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sbots3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                // Remove dead ones
                sbots4 = sbots4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sbots4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 26400;
                ...
                if (guard1.length < c.GUARD1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot);
                    o.team = -1;
                    //o.define(Class.basic);
                    o.define(Class.guardianku);
                    //o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    guard1.push(o);
                }
                // Remove dead ones
                guard1 = guard1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                guard1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                        o.skill.maintain();
                    }
                    if (o.skill.score > 26302) {
                        //o.skill.score -= 3698;
                ...
                // Remove dead ones
                guard2 = guard2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                guard2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                guard3 = guard3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                guard3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                guard4 = guard4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                guard4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (sboss1.length < c.SBOSS1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot2);
                    o.team = -1;
                    //o.define(Class.basic);
                ...
                // Remove dead ones
                sboss1 = sboss1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sboss1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                sboss2 = sboss2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sboss2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                sboss3 = sboss3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sboss3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                sboss4 = sboss4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                sboss4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                summon1 = summon1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                summon1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                summon2 = summon2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                summon2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                summon3 = summon3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                summon3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                summon4 = summon4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                summon4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (soverlord1.length < c.SOVERLORD1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot);
                    o.team = -1;
                ...
                // Remove dead ones
                soverlord1 = soverlord1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                soverlord1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                        o.skill.maintain();
                    }
                ...
                if (soverlord2.length < c.SOVERLORD2) {
                    let o = new Entity(room.randomType("bas2"));
                    o.color = 11;
                    o.define(Class.bossbot);
                    o.team = -2;
                ...
                // Remove dead ones
                soverlord2 = soverlord2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                soverlord2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                        o.skill.maintain();
                    }
                ...
                if (soverlord3.length < c.SOVERLORD3) {
                    let o = new Entity(room.randomType("bas3"));
                    o.color = 12;
                    o.define(Class.bossbot);
                    o.team = -3;
                ...
                // Remove dead ones
                soverlord3 = soverlord3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                soverlord3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                        o.skill.maintain();
                    }
                ...
                if (soverlord4.length < c.SOVERLORD4) {
                    let o = new Entity(room.randomType("bas4"));
                    o.color = 15;
                    o.define(Class.bossbot);
                    o.team = -4;
                ...
                // Remove dead ones
                soverlord4 = soverlord4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                soverlord4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                        o.skill.maintain();
                    }
                ...
                if (defender1.length < c.DEFENDER1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot);
                    o.team = -1;
                    //o.define(Class.basic);
                    o.define(Class.defender);
                    //o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    defender1.push(o);
                }
                // Remove dead ones
                defender1 = defender1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                defender1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (defender2.length < c.DEFENDER2) {
                    let o = new Entity(room.randomType("bas2"));
                    o.color = 11;
                    o.define(Class.bossbot);
                    o.team = -2;
                ...
                // Remove dead ones
                defender2 = defender2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                defender2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (defender3.length < c.DEFENDER3) {
                    let o = new Entity(room.randomType("bas3"));
                    o.color = 12;
                    o.define(Class.bossbot);
                    o.team = -3;
                ...
                // Remove dead ones
                defender3 = defender3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                defender3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (defender4.length < c.DEFENDER4) {
                    let o = new Entity(room.randomType("bas4"));
                    o.color = 15;
                    o.define(Class.bossbot);
                    o.team = -4;
                ...
                // Remove dead ones
                defender4 = defender4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                defender4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (hybrido1.length < c.HYBRIDO1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot);
                    o.team = -1;
                    //o.define(Class.basic);
                    o.define(Class.hybrido);
                    //o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    hybrido1.push(o);
                }
                // Remove dead ones
                hybrido1 = hybrido1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                hybrido1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                hybrido2 = hybrido2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                hybrido2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                hybrido3 = hybrido3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                hybrido3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                hybrido4 = hybrido4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                hybrido4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (tukki1.length < c.TUKKI1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot);
                    o.team = -1;
                    //o.define(Class.basic);
                    o.define(Class.Turkey_Mothership);
                    //o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    tukki1.push(o);
                }
                // Remove dead ones
                tukki1 = tukki1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                tukki1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                tukki2 = tukki2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                tukki2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                tukki3 = tukki3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                tukki3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                tukki4 = tukki4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                tukki4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (bird1.length < c.BIRD1) {
                    let o = new Entity(room.randomType("bas1"));
                    o.color = 10;
                    o.define(Class.bossbot);
                    o.team = -1;
                    //o.define(Class.basic);
                    o.define(Class.the_bird);
                    //o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    bird1.push(o);
                }
                // Remove dead ones
                bird1 = bird1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bird1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                bird2 = bird2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bird2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                bird3 = bird3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bird3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                bird4 = bird4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                bird4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                mother1 = mother1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                mother1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                mother2 = mother2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                mother2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                mother3 = mother3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                mother3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                mother4 = mother4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                mother4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (boso1.length < c.BOSO1) {
                    ...
                    //o.define(Class.basic);
                      ...
                      Class.elite_battle,
                      Class.elite_over,
                      Class.elite_spawner,]) : ran.choose([
                          Class.elite_destroyer, 
                          Class.elite_gunner, 
                          Class.elite_sprayer, 
                          Class.elite_battle,
                ...
                // Remove dead ones
                boso1 = boso1.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                boso1.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                boso2 = boso2.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                boso2.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                // Remove dead ones
                boso3 = boso3.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                boso3.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
                ...
                if (boso4.length < c.BOSO4) {
                    let o = new Entity(room.randomType("nest"));
                    //o.color = 15;
                    o.define(Class.bot28);
                    o.team = -100;
                    //o.define(Class.basic);
                      o.define((ran.dice(80)) ? ran.choose([
                      Class.kronos, 
                        Class.ragnarok]) : ran.choose([
                          Class.kronos, Class.ragnarok
                    ]));
                    //o.name += ran.chooseBotName();
                    o.refreshBodyAttributes();
                    boso4.push(o);
                ...
                // Remove dead ones
                boso4 = boso4.filter(e => { return !e.isDead(); });
                // Slowly upgrade them
                boso4.forEach(o => {
                    if (o.skill.score < 26400) {
                        o.skill.score += 100000;
...
// The big food function
  let makefood = (() => {
    ...
    function getFoodClass(level) {
      ...
      switch (level) {
        case 0:
          ...
          break;
        case 1:
          a = ran.dice(15) ? ran.choose([Class.greensquare, Class.rotsquare, Class.betaegg]) : Class.square;
          break;
        case 2:
          a = ran.dice(20) ? ran.choose([Class.greentriangle, Class.gem]) : Class.triangle;
          break;
        case 3:
          a = ran.dice(30) ? ran.choose([Class.greenpentagon, Class.betacube]) : Class.pentagon;
          break;
        case 4: a = Class.bigPentagon; break;
        case 5: a = Class.hugePentagon; break;
        case 6: a = Class.hexagon; break;
        case 7: a = ran.choose([Class.omegaPentagon, Class.heptagon]); break;
        case 8: a = Class.octagon; break;
        case 9: a = Class.nonagon; break;
        case 10: a = Class.decagon; break;
        case 11: a = Class.icosagon; break;
        case 12: a = Class.megapoly; break;
    ...
    let makeGroupedFood = () => {
      ...
      placeNewFood(spot, 0.1 * room.width, 0);
      placeNewFood(spot, 0.08 * room.width, 1, true);
      placeNewFood(spot, 0.04 * room.width, 2, true);
      placeNewFood(spot, 0.02 * room.width, 3, true);
      placeNewFood(spot, 0.01 * room.width, 4, true);
      return true;
    };
    let makeCornerFood = () => {
    ...
    let makeNestFood = () => {
      ...
      if (!spot) return false;
      placeNewFood(spot, 0.1 * room.width, 3, true);
      placeNewFood(spot, 0.15 * room.width, 2, true);
      placeNewFood(spot, 0.25 * room.width, 1, true);
      placeNewFood(spot, 0.05 * room.width, 4, true);
      return true;
    };
    ...
    return () => {
      ...
      food = entities
        .map((instance) => {
          ...
          try {
            if (instance.type === "tank" || instance.type === "spike") {
              census.tank++;
      ...
      // Sum it up
      let maxFood = 10;//1 + room.maxFood + 1 * census.tank;
...
const spawner = (() => {
  const spawnerConfig = require("./lib/spawner");

  let defineTeam = (e, team) => {
    switch (team) {
      case "random":
        {
          e.team = ran.irandomRange(1, 4);
          e.color = [10, 11, 12, 15][e.team - 1];
        }
        break;
      case "blue":
        {
  ...
  let spawn = (spawner, loc) => {
    let e = new Entity(loc);
    if (spawner.bot) {
      e.define(Class.bot);
      e.name += ran.chooseBotName();
    }
    e.define(Class[ran.choose(spawner.tank)]);
    defineTeam(e, spawner.team);
    if (spawner.controllers) {
      let toAdd = [];
      spawner.controllers.forEach((ioName) => {
        toAdd.push(eval("new io_" + ioName + "(e)"));
      });
      e.addController(toAdd);
    }
    e.refreshBodyAttributes();
    spawner.spawnedEntities.push(e);
  };

  let tryGetCenterLocation = (censusLocations) => {
    let loc;
    let i = censusLocations.length * 10;
    while (true) {
      let selection = ran.choose(censusLocations);
      loc = {
        x: selection.x,
        y: selection.y,
      };
      i--;
      if (!i) break;
      if (!dirtyCheck(loc, 50)) break;
    }
    return loc;
  };

  let tryGetRandomLocation = (censusLocations) => {
    let loc;
    let i = censusLocations.length * 10;
    while (true) {
      let selection = ran.choose(censusLocations);
  ...
  class Spawner {
    constructor(v) {
      this.active = true;
      this.interval = 5;
      this.location = ["norm", "nest", "roid"];
      this.tank = ["basic"];
      this.maxAmount = 10;
      this.spawningPerInterval = 1;
    ...
    trigger() {
      logs.spawner.set();
      this.spawnedEntities = this.spawnedEntities
        .map((e) => {
          if (e.valid()) {
            return e;
          }
      ...
      let censusLocations = [];
      for (let type of this.location) {
        if (!room[type] || !room[type].length) continue;
        censusLocations.push(...room[type]);
      }

      let spawningCount = 0;
      if (censusLocations.length > 0) {
        if (this.position === "center") {
          let loc = tryGetCenterLocation(censusLocations);
          if (loc) {
            for (let i = 0; i < this.spawningPerInterval; i++) {
              if (this.spawnedEntities.length >= this.maxAmount) break;
              spawn(this, loc);
              spawningCount++;
            }
          }
        } else if (this.position === "random") {
          for (let i = 0; i < this.spawningPerInterval; i++) {
            if (this.spawnedEntities.length >= this.maxAmount) break;
            let loc = tryGetRandomLocation(censusLocations);
            if (loc) {
              spawn(this, loc);
              spawningCount++;
            }
          ...
          throw new Error("Spawn position error.");
        }
      }

      if (spawningCount > 0 && this.message) {
        sockets.broadcast(this.message);
      }
      logs.spawner.mark();
    }
    ...
    activate(b) {
      ...
      if (b) {
        this.intervalID = setInterval(() => {
          this.trigger();
        }, Math.max(this.interval * 10, room.cycleSpeed));
      } else {
  ...
  let o = {};
  for (const [key, value] of Object.entries(spawnerConfig)) {
    o[key] = new Spawner(value);
  }
  return o;
})();
...
let server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    case "/":
      res.writeHead(200);
      res.end(
        `<!DOCTYPE html><h3>Arras</h3><button onclick="location.href = 'http://arras.cx/#host=' + location.host +'&mode=eeWar'">Open</button>`
      );
      break;
    case "/mockups.json":
...
setInterval(maintainloop, 800);
setInterval(speedcheckloop, 4000);
